...
Record a numeric score — ASSIGNMENT 85 should return 201
    ...
    And a new Student is prepared for this test
    When a POST request is made to "/api/v1/grade-records" with score 85
...
Record a numeric score — ASSIGNMENT negative score should return 400
    [Documentation]    US-05-01 — Reused from: grade_recording.feature
    Given a GradeItem with ID "${GRADE_ITEM_ID}" of type "ASSIGNMENT" exists
    When a POST request is made to "/api/v1/grade-records" with score -5
...
Record a numeric score — CLASSROOM_PERFORMANCE negative bonus should return 201
    ...
    When a POST request is made to "/api/v1/grade-records" for CLASSROOM_PERFORMANCE item with score -2
    Then the response code should be 201

Record a numeric score — ATTENDANCE 1 should return 201
    [Documentation]    US-05-01 — Reused from: grade_recording.feature
    Given a GradeItem with type "ATTENDANCE" is prepared for this test
    When a POST request is made to "/api/v1/grade-records" for ATTENDANCE item with score 1
    Then the response code should be 201

Record a numeric score — ATTENDANCE 0 should return 201
    [Documentation]    US-05-01 — Reused from: grade_recording.feature
    Given a GradeItem with type "ATTENDANCE" is prepared for this test
    When a POST request is made to "/api/v1/grade-records" for ATTENDANCE item with score 0
    Then the response code should be 201

Record a numeric score — REPORT 75 should return 201
    [Documentation]    US-05-01 — Reused from: grade_recording.feature
    Given a GradeItem with type "REPORT" is prepared for this test
    When a POST request is made to "/api/v1/grade-records" for REPORT item with score 75
    Then the response code should be 201

Record a numeric score — OTHER 45 should return 201
    [Documentation]    US-05-01 — Reused from: grade_recording.feature
    Given a GradeItem with type "OTHER" is prepared for this test
    When a POST request is made to "/api/v1/grade-records" for OTHER item with score 45
    Then the response code should be 201
...
Record Attendance with status PRESENT maps to score 1.0
    ...
    Given a GradeItem with type "ATTENDANCE" is prepared for this test
    When a POST request is made to "/api/v1/grade-records" with attendance_status "PRESENT"
    Then the response code should be 201
...
Record Attendance with status ABSENT maps to score 0.0
    [Documentation]    US-05-01 AC2 — Reused from: grade_recording.feature
    Given a GradeItem with type "ATTENDANCE" is prepared for this test
    When a POST request is made to "/api/v1/grade-records" with attendance_status "ABSENT"
    Then the response code should be 201
...
Record Attendance with status EXCUSED maps to score 0.5
    [Documentation]    US-05-01 AC2 — Reused from: grade_recording.feature
    Given a GradeItem with type "ATTENDANCE" is prepared for this test
    When a POST request is made to "/api/v1/grade-records" with attendance_status "EXCUSED"
    Then the response code should be 201
...
Update an existing GradeRecord
    ...
    When a PUT request is made to "/api/v1/grade-records/${GRADE_RECORD_ID}" with score 90
...
Update a non-existent GradeRecord returns 404
    [Documentation]    US-05-02 — Reused from: grade_recording.feature
    When a PUT request is made to "/api/v1/grade-records/00000000-0000-0000-0000-000000000000" with score 80
...
Upload an Attachment for a GradeRecord
    ...
    Given a GradeRecord with ID "${GRADE_RECORD_ID}" exists
    When a POST request is made to "/api/v1/grade-records/${GRADE_RECORD_ID}/attachments" with a 5MB file
    Then the response code should be 201
    And the response should contain a valid UUID for "id"
...
Reject Attachments exceeding 10MB size limit
    ...
    Given a GradeRecord with ID "${GRADE_RECORD_ID}" exists
    When a POST request is made to "/api/v1/grade-records/${GRADE_RECORD_ID}/attachments" with a 15MB file
    Then the response code should be 400
...
Reject Attachment when limit of 5 per GradeRecord is reached
    ...
    When a POST request is made to "/api/v1/grade-records/${FULL_RECORD_ID}/attachments" with a 1MB file
    Then the response code should be 400
...
Initialize Grade Recording Suite
    ...
    # Step 1: Semester
    ${s_payload}=    Create Dictionary    semesterName=AutoTest-GradeRecSuite-Semester    startDate=2024-09-01    endDate=2025-01-31
    ...
    # Step 2: Class
    ${c_payload}=    Create Dictionary    className=AutoTest-GradeRecSuite-Class
    ...
    # Step 3: Student
    ${st_payload}=    Create Dictionary    studentId=S_RecSuite_1    studentNumber=${1}    studentName=AutoTest-SuiteStudent    email=recsuite1@gmail.com
    ...
    ${i_payload}=    Create Dictionary    itemName=AutoTest-SuiteAssignment    itemType=ASSIGNMENT    maxScore=${100}    weight=${0.20}
    ...
    ${r_payload}=    Create Dictionary    gradeItemId=${GRADE_ITEM_ID}    studentId=${STUDENT_ID}    score=${80}
...
a GradeRecord with ID "${record_id}" exists
    ${resp}=    GET On Session    score_api    /api/v1/grade-records/${record_id}    expected_status=any
    Should Be Equal As Strings    ${resp.status_code}    200
...
a GradeRecord with 5 existing attachments is prepared
    ...
    ${st_payload}=    Create Dictionary    studentId=S_Temp_Att_1    studentNumber=${100}    studentName=Temp-Attachment-Student    email=tempatt1@gmail.com
    ${st_resp}=    POST On Session    score_api    ${STUDENTS_BASE}    json=${st_payload}
    ${temp_student_id}=    Set Variable    ${st_resp.json()}[id]
    ${r_payload}=    Create Dictionary    gradeItemId=${GRADE_ITEM_ID}    studentId=${temp_student_id}    score=${50}
    ${r_resp}=    POST On Session    score_api    /api/v1/grade-records    json=${r_payload}
    Should Be Equal As Strings    ${r_resp.status_code}    201
...
a new Student is prepared for this test
    ${st_payload}=    Create Dictionary    studentId=S_Temp_Rec_1    studentNumber=${101}    studentName=Temp-Record-Student    email=temprec1@gmail.com
    ${st_resp}=    POST On Session    score_api    ${STUDENTS_BASE}    json=${st_payload}
    Should Be Equal As Strings    ${st_resp.status_code}    201
    Set Test Variable    ${TEMP_STUDENT_ID}    ${st_resp.json()}[id]

# ---------------------------------------------------------------------------
# When Steps
# ---------------------------------------------------------------------------
a POST request is made to "/api/v1/grade-records" with score ${score}
    [Documentation]    POST /api/v1/grade-records with numeric score
    ...    UI: score-input (grade-entry-board.ui-manifest.json)
    ${target_student}=    Get Variable Value    \${TEMP_STUDENT_ID}    ${STUDENT_ID}
    ${payload}=    Create Dictionary
...
a POST request is made to "/api/v1/grade-records" for ${item_type} item with score ${score}
    [Documentation]    POST /api/v1/grade-records using ${TEMP_ITEM_ID} created in Given step
    ${payload}=    Create Dictionary
...
a PUT request is made to "/api/v1/grade-records/${record_id}" with score ${score}
    [Documentation]    PUT /api/v1/grade-records/{id}
    ...    UI: score-input (grade-entry-board.ui-manifest.json)
    ${payload}=    Create Dictionary
    ...    gradeItemId=${GRADE_ITEM_ID}
    ...    studentId=${STUDENT_ID}
    ...    score=${score}
    ${resp}=    PUT On Session    score_api    /api/v1/grade-records/${record_id}    json=${payload}    expected_status=any
    Set Test Variable    ${RESPONSE}    ${resp}
