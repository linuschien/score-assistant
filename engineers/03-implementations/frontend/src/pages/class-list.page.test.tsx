...
describe('ClassListPage', () => {
  ...
  it('opens modal on click of 建立班級', async () => {
    render(
      <QueryClientProvider client={queryClient}>
        <JSONUIProvider registry={componentRegistry} store={store} handlers={testHandlers as any}>
          <ClassListPage />
    ...
    const trigger = await screen.findByRole('button', { name: /建立班級/i });
    expect(trigger).toBeInTheDocument();
    
    fireEvent.click(trigger);
    
    expect(await screen.findByText(/建立 \/ 編輯班級/i)).toBeInTheDocument();
  });
});
